// ═══════════════════════════════════════
//  ASTRA AI — api/chat.js
//  Deploy ke Vercel sebagai Serverless Function
// ═══════════════════════════════════════

const GROQ_KEYS = [
  "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",
  "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",
  "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",
...
async function callGroq(messages, attempt = 0) {
  if (attempt >= GROQ_KEYS.length) {
    throw new Error("Semua API key sedang rate limited, coba lagi nanti.");
  }
  ...
  if (groqRes.status === 429) {
    console.warn(`Key ke-${attempt + 1} rate limited, mencoba key berikutnya...`);
